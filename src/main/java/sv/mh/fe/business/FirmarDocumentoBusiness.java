...
public class FirmarDocumentoBusiness {
	...
	public String firmarJSONBase64(String certificadoB64, String password, String contenido) throws Exception {
		...
		try {
			// PRIORIDAD: Intentar como XML (CertificadoMH) - más común y rápido
			...
			// FALLBACK: Intentar como PKCS12 si no es XML
			try {
				...
				if (alias == null) {
					throw new Exception("No se encontró un alias en el certificado PKCS12");
				}
				...
				jws.setKey(key);
				return jws.getCompactSerialization();
			} catch (Exception pkcsEx) {
				throw new Exception("El certificadoB64 no es un XML de CertificadoMH válido ni un PKCS12 válido. Error XML: " + xmlEx.getMessage() + ", Error PKCS12: " + pkcsEx.getMessage());
			}
		}
	}
}
